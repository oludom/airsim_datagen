...
class RecurrentRaceTracksLoader(RacetrackLoader):
    def __init__(self, dataset_basepath: str, dataset_basename: str, raceTrackName: str, localTrajectoryLength: int = 3, skipFirstXImages = 2, skipLastXImages=0, sequence_length=5):
        ...
        self.skipFirstXImages = 0 if skipFirstXImages == 0 else skipFirstXImages * 1
        self.skipLastXImages = -1 if skipLastXImages == 0 else skipLastXImages * -1
        print(skipFirstXImages)
    def __iter__(self):
        jump = 1
        # for i in range(3,len(self.data[:self.skipLastXImages]),self.sequence_length):
        for i in range(self.skipFirstXImages,len(self.data[:-self.sequence_length]),jump):
            # x = i + self.sequence_length
            # img_num =  + i
